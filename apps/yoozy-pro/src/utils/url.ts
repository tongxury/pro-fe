/**
 * 从字符串中提取小红书链接
 * @param {string} text - 输入的字符串
 * @returns {string|null} - 找到的链接或null
 */
export function extractLink(text: string): string {

    // 定义要查找的两种前缀
    const prefixes = [
        "https://v.douyin.com/",
        "http://xhslink.com/",
        "https://xhslink.com/",
        "https://www.xiaohongshu.com/discovery/item/",
        "https://www.xiaohongshu.com/explore/"
    ];

    // 尝试找到任一前缀
    for (const prefix of prefixes) {
        const startIndex = text.indexOf(prefix);

        // 如果找到前缀
        if (startIndex !== -1) {
            // 从前缀开始位置截取
            let link = text.substring(startIndex);

            // 查找可能的结束字符
            const endChars = [' ', '\n', '\t', ',', '，', ';', '；', '。', '!', '！', '？', '"', "'", ')', '）', ']', '】', '}', '」', '』'];

            // 找到最早的结束字符位置
            let endIndex = link.length;
            for (const char of endChars) {
                const charIndex = link.indexOf(char);
                if (charIndex !== -1 && charIndex < endIndex) {
                    endIndex = charIndex;
                }
            }

            // 截取到结束字符位置
            link = link.substring(0, endIndex);

            return link;
        }
    }

    return "";
}

/**
 * 从文本中解析出所有 https:// 链接（更严格的 URL 格式）
 * @param {string} text - 要解析的文本
 * @returns {string[]} - 返回所有找到的 https:// 链接数组
 */
export function extractDouyinLink(text: string): string {
    // 更严格的 URL 正则表达式
    // 匹配 https:// 后跟有效的 URL 字符，直到遇到空白字符或标点符号（保留常见 URL 字符）
    const regex = /https:\/\/v\.douyin\.com\/[^\s]+/g;
    const matches = text.match(regex);
    return matches?.[0] || '';
}
