import { addTemplates, deleteTemplate, listTemplates, refreshTemplate } from "@/api/template";
...
import { Button, List, Pagination, Select, Skeleton, Space } from 'antd';
...
export default function Templates() {
    ...
    const { list, total, reload, initialLoading } = useQueryListWithPolling<any, any>({
        ...
        params: {
            ...params,
            returnFields: 'url,coverUrl,status,commodity.name'
        },
        key: item => item._id,
    ...
    const onAdd = (results: any[]) => {
        addTemplates(results?.map(x => ({ url: x.videoUrl, coverUrl: x.coverUrl }))).then(() => {
            reload()
        });
    };
