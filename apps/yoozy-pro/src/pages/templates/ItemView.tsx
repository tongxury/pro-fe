            ...
            <div className="group relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100 flex flex-col h-full cursor-pointer">
                ...
                >
                    {isVideoPlaying ? (
                        ...
                        >
                            <source src={cached(hasVideo)} type="video/mp4" />
                            您的浏览器不支持视频播放
                        </video>
                    ) : (
                        <>
                            <img
                                alt={data.title}
                                src={cached(data.coverUrl || data?.segments?.[0]?.highlightFrames?.[0]?.url)}
                            ...
                            <div className="absolute top-3 left-3">
                                {data.status !== 'completed' ? (
                                    <span className="px-2 py-1 rounded-md bg-primary/80 backdrop-blur-sm text-white text-xs font-medium border border-blue-400/30">
                                        处理中 预计5分钟完成
                                    </span>
                                ) : (
                                    <>
