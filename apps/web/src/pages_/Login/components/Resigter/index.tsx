import React, { useState } from "react";
import { Button, Form, Input, message } from "antd";

import RegisterCode from "../RegisterCode";
import { addEventLog, registerByEmail } from "@/api/api";
import { useTranslations } from "next-intl";

export default function Resigter(props: any) {
    const t = useTranslations("Login");

    const { backInit } = props;
    const [isregister, setIsregister] = useState(false);
    const [email, setEmail] = useState("");
    const [pwd, setPwd] = useState("");
    const onFinish = (values: any) => {
        setPwd(values.pwd);
        register(values);
    };
    const onFinishFailed = (errorInfo: any) => {
        console.log("Failed:", errorInfo);
    };
    const register = async (params: any) => {
        addEventLog({
            id: "sign_up",
            version: document.body.getAttribute("xtips-version") || ""
        });
        try {
            const data = await registerByEmail(params);

            if ((data.code || 0) !== 0) {
                message.error(data.message || data.data);
                setIsregister(false);
            } else {
                //
                setIsregister(true);
                setEmail(params.email);
            }
        } catch (err) {
            addEventLog({
                id: "register_error",
                version: document.body.getAttribute("xtips-version") || "",
                params: {
                    error: err
                }
            });
        }
    };
    return (
        <>
            {isregister ? (
                <RegisterCode email={email} pwd={pwd}></RegisterCode>
            ) : (
                <div className="email-login">
                    <div className="back" onClick={backInit}>
                        <svg
                            data-v-140ca179=""
                            width="21"
                            height="20"
                            viewBox="0 0 21 20"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                        >
                            <path
                                data-v-140ca179=""
                                d="M10.7962 11.6383L15.5347 6.89982C15.6882 6.74628 15.8581 6.66773 16.0444 6.66418C16.2306 6.66061 16.404 6.73916 16.5647 6.89982C16.7253 7.06047 16.8057 7.23213 16.8057 7.4148C16.8057 7.59748 16.7222 7.77227 16.5553 7.93918L11.4146 13.0798C11.3219 13.1651 11.2245 13.2291 11.1223 13.2718C11.02 13.3144 10.9095 13.3357 10.7907 13.3357C10.6718 13.3357 10.5613 13.3144 10.4591 13.2718C10.3568 13.2291 10.2631 13.1651 10.1778 13.0798L5.03711 7.93918C4.87732 7.77939 4.80027 7.61099 4.80596 7.43399C4.81163 7.257 4.8948 7.08819 5.05546 6.92755C5.21611 6.76688 5.38777 6.68655 5.57044 6.68655C5.75312 6.68655 5.92478 6.76688 6.08542 6.92755L10.7962 11.6383Z"
                                fill="#869fb8"
                            ></path>
                        </svg>
                        Back
                    </div>

                    <Form
                        name="basic"
                        initialValues={{ remember: true }}
                        onFinish={onFinish}
                        onFinishFailed={onFinishFailed}
                        autoComplete="off"
                    >
                        <Form.Item
                            label=""
                            name="email"
                            rules={[
                                { required: true, message: t("login_enter_email") },

                                {
                                    pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                                    message: t("login_valid_email")
                                }
                            ]}
                        >
                            <Input placeholder={t("login_valid_email")} className="email" />
                        </Form.Item>
                        <Form.Item
                            label=""
                            name="pwd"
                            rules={[
                                { required: true, message: t("login_enter_password") },
                                {
                                    pattern: /^(?=.*[a-zA-Z])(?=.*\d).{6,}$/,
                                    message: t("login_valid_password")
                                }
                            ]}
                        >
                            <Input.Password placeholder={t("login_password")} className="pwd" />
                        </Form.Item>
                        <Form.Item
                            label=""
                            name="nickname"
                            rules={[
                                { required: true, message: t("login_enter_name") },
                                {
                                    min: 2,
                                    message: t("login_valid_name")
                                }
                            ]}
                        >
                            <Input placeholder={t("login_name")} className="email" />
                        </Form.Item>
                        <Form.Item>
                            <Button type="primary" htmlType="submit" className="email-btn">
                                {t("login_signUp")}
                            </Button>
                        </Form.Item>
                    </Form>
                </div>
            )}
        </>
    );
}
