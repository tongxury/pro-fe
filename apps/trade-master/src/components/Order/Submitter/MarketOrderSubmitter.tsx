...
import {sendTrade} from "@/api/trade.ts";
import {useInterval} from "ahooks";

function MarketOrderSubmitter({
                                  params: {token, side, amount, category, autoSell},
                                  onSubmitted,
                                  children,
                                  styles,
                              }: {
    ...
    const onSubmit = () => {
        ...
        notify.open({
            ...
            message: <XText size={17}>{t(`inProgress_${side}`)}</XText>,
            closable: false,
            showProgress: true,
            duration: 20,
        ...
        sendTrade({token: token._id || token.id, side, amount, category, settings: {...settings, metadata: undefined, autoSell}})
            .then(result => {
                ...
                if (result.code) {
                    ...
                    notify.open({
                        ...
                        message: <XText size={17}>{t('orderSuccess')}</XText>,
                        // description: <XText size={14} color={themeVars.colorTextL2}>{t(result.code)}</XText>,
                        showProgress: true,
                        duration: 2,
                    });
    ...
    return <>
        ...
        {cloneElement(children, {
            ...
            style: {
                ...
                background: amount ? color : undefined
            }
        })}
    </>
...
export default MarketOrderSubmitter
