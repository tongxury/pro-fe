...
let [repo, hubId, cat, env, version] = args;
...
if (version === 'auto') {
    ...
    console.log(`Using timestamp version: ${version}`);
}
...
const execute = async () => {
    try {
        ...
        //
        await runCommand(`docker buildx build --platform linux/amd64 -t yoozy-cn-shanghai.cr.volces.com/yoozy/${hubId}:${version} --build-arg REPO=${repo} --build-arg APP_ENV=${env} -f Dockerfile-${cat} . --push`)
        ...
        // await runCommand(`docker tag ${registryTag} yoozy-cn-shanghai.cr.volces.com/yoozy/${hubId}:${version}`)
        // await runCommand(`docker push yoozy-cn-shanghai.cr.volces.com/yoozy/${hubId}:${version}`)

        if (repo === 'yoozy-admin' && version !== 'test') {
            await runCommand(`
curl -X PUT \
    -H "content-type: application/json" \
    -H "Cookie: KuboardUsername=admin; KuboardAccessKey=4342pex2bf55.kkx8hf6z665b85fsf38ichrnfrc238kz" \
    -d '{"kind":"deployments","namespace":"prod","name":"admin-site","images":{"yoozy-cn-shanghai.cr.volces.com/yoozy/proj-admin-fe":"yoozy-cn-shanghai.cr.volces.com/yoozy/proj-admin-fe:${version}"}}' \
    "http://118.196.63.209:18060/kuboard-api/cluster/vke-shanghai/kind/CICDApi/admin/resource/updateImageTag"
        `)
        }

        if (repo === 'yoozy-pro' && version !== 'test') {
            await runCommand(`
curl -X PUT \
    -H "content-type: application/json" \
    -H "Cookie: KuboardUsername=admin; KuboardAccessKey=4342pex2bf55.kkx8hf6z665b85fsf38ichrnfrc238kz" \
    -d '{"kind":"deployments","namespace":"prod","name":"pro-site","images":{"yoozy-cn-shanghai.cr.volces.com/yoozy/proj-pro-fe":"yoozy-cn-shanghai.cr.volces.com/yoozy/proj-pro-fe:${version}"}}' \
    "http://118.196.63.209:18060/kuboard-api/cluster/vke-shanghai/kind/CICDApi/admin/resource/updateImageTag"
            `)
        }
        ...
        curl -X PUT \
            ...
            -H "Cookie: KuboardUsername=admin; KuboardAccessKey=4342pex2bf55.kkx8hf6z665b85fsf38ichrnfrc238kz" \
            -d '{"kind":"deployments","namespace":"beta","name":"pro-site"}' \
    ...
    -H "Content-Type: application/yaml" \
    -H "Cookie: KuboardUsername=admin; KuboardAccessKey=4342pex2bf55.kkx8hf6z665b85fsf38ichrnfrc238kz" \
    -d '{"kind":"deployments","namespace":"beta","name":"admin-site"}' \
    "http://118.196.63.209:18060/kuboard-api/cluster/vke-shanghai/kind/CICDApi/admin/resource/restartWorkload"
                `)
